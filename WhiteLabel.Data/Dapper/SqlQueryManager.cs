...
namespace WhiteLabel.Data.Dapper
{
    public class SqlQueryManager : IQueryManager
    {
        private static readonly Func<Assembly> AssemblyProvider;
        private static readonly Func<string> ResourceNamePrefixProvider;
        private static readonly ConcurrentDictionary<string, string> QueryCache = new ConcurrentDictionary<string, string>();
        private static readonly Func<string[]> ResourceNamesProvider;
        ...
        public string GetQuery(string queryName)
        {
            ...
            string query;
            var resourceName = ResourceNamePrefixProvider() + queryName + ".sql";
            if (!QueryCache.TryGetValue(resourceName, out query))
            {
                if (ResourceNamesProvider().Any(x => x.Equals(resourceName, StringComparison.InvariantCulture)))
                {
                    using (var stream = AssemblyProvider().GetManifestResourceStream(resourceName))
                        if (stream != null)
