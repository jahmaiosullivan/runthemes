    ...
    initialize: function() {
        $('#fileupload').fileupload({
            ...
            completed: function(e, data) {
                var uploadFile = data.getFilesFromResponse(data)[0];
                $('#uploadedImage').attr('src', uploadFile.url);
                $('#uploadedImage').attr('data-uploaded', 'true');
            },
            disableImagePreview: false
        });
    },

    wire: function(uploadSectionId,  picListId) {
        'use strict';
        ...
        $('#' + uploadSectionId).fileupload({
            ...
            .on('fileuploaddone', function(e, data) {
                var previewElement = $('<li/>').appendTo('ul#' + picListId).attr('data-filename', data.result.files[0].imageUrl);
                $(previewElement).prepend(data.files[0].preview).prepend('<a href="#" class="delete"><i class="fa fa-trash-o"></i></a>');
            })
...
HobbyClue.Location = {
    ...
    SetCurrentCity: function() {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(HobbyClue.Location.City);
        }
    },
