	...
	public class TopicsDataAccess : BaseDataAccess, ITopicsDataAccess
	{
		...
		public List<Topic> GetLatest()
		{
			...
			foreach (DataRow dr in dt.Rows)
			{
				...
				t.User = new User(dr.Get<Guid>("UserId"), dr.Get<string>("UserName"));
		...
		public Topic Get(int id)
		{
			...
			if (dr != null)
			{
				...
				t.User = new User(dr.Get<Guid>("UserId"), dr.Get<string>("UserName"));
				t.User.Photo = dr.GetNullableString("UserPhoto");
				t.User.RegistrationDate = dr.GetDate("UserRegistrationDate");
				t.User.Signature = dr.GetNullableString("UserSignature");
				t.User.Role = dr.Get<UserRole>("UserGroupId");
				t.User.RoleName = dr.GetString("UserGroupName");
				t.Forum = new Forum();
		...
		protected virtual List<Topic> ParseTopicsForFullList(DbCommand comm)
		{
			...
			foreach (DataRow dr in dt.Rows)
			{
				...
				if (!dr.IsNull("MessageCreationDate"))
				{
					...
					t.LastMessage.User = new User(dr.Get<Guid>("MessageUserId"), dr.GetString("MessageUserName"));
				}
				list.Add(t);
			}
			return list;
		...
		public void Add(Topic topic, string ip)
		{
			...
			comm.AddParameter<Guid>(this.Factory, "UserId", topic.User.Guid);
			comm.AddParameter<string>(this.Factory, "TopicTags", topic.Tags != null ? topic.Tags.ToString() : null);
		...
		public void Edit(Topic topic, string ip)
		{
			...
			comm.AddParameter<string>(this.Factory, "TopicTitle", topic.Title);
			comm.AddParameter<string>(this.Factory, "TopicDescription", topic.Description);
			comm.AddParameter<Guid>(this.Factory, "UserId", topic.User.Id);
			comm.AddParameter<string>(this.Factory, "TopicTags", topic.Tags != null ? topic.Tags.ToString() : null);
			comm.AddParameter(this.Factory, "TopicOrder", DbType.Int32, topic.IsSticky ? 1 : (int?) null);
			comm.AddParameter<string>(this.Factory, "Ip", ip);
			comm.AddParameter(this.Factory, "ReadAccessGroupId", DbType.Int16, topic.ReadAccessRole);
        ...
        public void Move(int id, int forumId, Guid userId, string ip)
		{
		...
		{
			DbCommand comm = GetCommand("SPTopicsGetUnansweredCount");
		...
		/// <summary>
		/// Marks the topic as inappropriate
		/// </summary>
		/// <returns>If the message was flagged or not (if it wasn't already flagged by the same ip).</returns>
		public bool Flag(int topicId, string ip)
		{
			var comm = GetCommand("SPMessagesFlag");
			comm.AddParameter(Factory, "TopicId", topicId);
			comm.AddParameter<int?>(Factory, "MessageId", null);
			comm.AddParameter(Factory, "Ip", ip);

			return comm.SafeExecuteNonQuery() > 0;
		}

		public List<Topic> GetUnanswered(int firstMsg, int amount)
		{
