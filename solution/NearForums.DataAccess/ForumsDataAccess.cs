	...
	public class ForumsDataAccess : BaseDataAccess, IForumsDataAccess
	{
		...
		public List<ForumCategory> GetList(UserRole? role)
		{
			...
			foreach (DataRow dr in dt.Rows)
			{
				...
				Forum f = ParseForumWithTopicDataRow(dr);
				f.Category = category;
        ...
        public void Add(Forum f, Guid userId)
		{
			...
			comm.AddParameter(this.Factory, "UserId", DbType.Guid, userId);
		...
		protected virtual Forum ParseForumWithTopicDataRow(DataRow dr)
		{
			Forum f = ParseForumDataRow(dr);

			if (!dr.IsNull("TopicCreationDate"))
			{
				f.LastTopic = new Topic();
				f.LastTopic.User = new User(dr.Get<Guid>("UserId"), dr.Get<string>("UserName"));
				f.LastTopic.Id = dr.Get<int>("TopicId");
				f.LastTopic.Date = dr.GetDate("TopicCreationDate");
				f.LastTopic.Title = dr.GetString("TopicTitle");
				f.LastTopic.ShortName = dr.GetString("TopicShortName");
			}
        ...
        public void Edit(Forum f, Guid userId)
		{
