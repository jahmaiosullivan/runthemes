...
namespace NearForums.Web.Controllers
{
	public class AuthenticationController : BaseController
	{
		...
		public ActionResult Login(string returnUrl, UserRole? role)
		{
			...
			if (Config.AuthenticationProviders.SSOOpenId.IsDefined)// Auto redirect to OpenId Login if only option available.
			{
				if (!Config.AuthenticationProviders.CustomDb.IsDefined && !Config.AuthenticationProviders.Facebook.IsDefined
					&& !Config.AuthenticationProviders.Twitter.IsDefined && !Config.AuthenticationProviders.FormsAuth.IsDefined)
				{
					return RedirectToAction("OpenIdStartLogin", new { returnUrl });
				}
			}
		...
		public ActionResult OpenIdStartLogin(string openidIdentifier, string returnUrl)
		{
			...
			if (Identifier.TryParse(openidIdentifier, out id))
			{
				...
				var returnAbsoluteUrl = new Uri(Request.Url, Url.Action("OpenIdFinishLogin", new { returnUrl }));

				var realmUrl = new Uri(Request.Url, "/");

				var openid = new OpenIdRelyingParty(null);
